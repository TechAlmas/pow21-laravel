<?php 
    ...
    class ScrapeCannabisController extends \crocodicstudio\crudbooster\controllers\CBController {
    ...
    public function scrapeCategoryDetails(Request $request,$cat_id){
        ...
        $crawler->filter('.productgrid--items .productgrid--item')->each(function ($node) use($category) {
            ...
            if($dom){
				...
				$product_name->source 	  		= 'cannabisannabisstores';
				...
				$product_type->source 		  = 'cannabisannabisstores';
                ...
                $crawler1->filter('.product--container')->each(function ($node1) use($category,$product1,$product_name) {
                    ...
                    if($chartics){
                        ...
                        foreach($chartics as $c_key=>$chartic){
							...
							if($meta_key == 'producer'){
								...
								$master_brand->source = 'cannabisannabisstores';
								$master_brand->save();
						...
						$type_data = '';
						$brand_data = '';
						foreach ($charitcs_array as $ckey => $charitc) {
							if (strtolower($charitc['key']) == 'type') {
								$type_data = $charitc;
							} 
							if (strtolower($charitc['key']) == 'brand') {
								$brand_data = $charitc;
								break;
							} 
						}
                        ...
                        if(!empty($brand_data)){
							$master_brand1 = MasterBrand::firstOrNew(['name'=>trim($brand_data['value'])]);
							$master_brand1->name = trim($brand_data['value']);
							$master_brand1->source = 'cannabisannabisstores';
							$master_brand1->save();
						...
						if(!empty($type_data)){
							$master_strain = MasterStrain::firstOrNew(['slug'=>$product1->slug,'source'=>'cannabisannabisstores']);
							$master_strain->name = $product1->product_title;
							$master_strain->category = $type_data['value'];
                    ...
                    if($variants){
						...
						if(!empty($variants_prices)){
							if($master_strain){
								$master_location_strain = MasterLocationStrain::firstOrNew(['slug'=>$master_strain->slug,'source'=>'cannabisannabisstores','strain_id'=>$master_strain->id]);
								...
								$master_location_strain->source = 'cannabisannabisstores';
								...
								if(!empty($type_data)){
									$master_strain = MasterStrain::firstOrNew(['slug'=>$product1->slug,'source'=>'cannabisannabisstores']);
									$master_strain->name = $product1->product_title;
									$master_strain->category = html_entity_decode($type_data['value']);
									$master_strain->description = $product1->product_description;
									$master_strain->slug = $product1->slug;
									$master_strain->source = 'cannabisannabisstores';
									$master_strain->save();
		...
		if($scrape_status){
			...
			//return redirect('admin/cannabis-strains?cat_id='.$category->id);
		}else{
