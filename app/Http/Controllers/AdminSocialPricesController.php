<?php namespace App\Http\Controllers;
	...
	class AdminSocialPricesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col = [];
			//$this->col[] = ["label"=>"City","name"=>"city_id","join"=>"master_cities,city"];
			$this->col[] = ["label"=>"City","name"=>"city_id","join"=>"master_cities,city","callback" => function($row){

				

				$city = DB::table("master_cities")
					->where("id","=",(int) $row->city_id)
					->first();

				//echo "<pre>"; print_r($city);exit;
				
				if($city->id){
					return $city->city.", ". $city->state_code;
				}else{
					return $row->city_id;
				}
			}];
			//$this->col[] = ["label"=>"Strain","name"=>"strain_id","join"=>"master_strains,name"];
			$this->col[] = ["label"=>"Strain","name"=>"strain_id","join"=>"master_strains,name", "callback" => function($row){
					if($row->strain_id == 0)
						return "MariJuana";
					else
						return $row->master_strains_name;
			}];
			$this->col[] = ["label"=>"Mass","name"=>"mass_id","join"=>"master_mass,name"];
