<?php namespace App\Http\Controllers;
		...
		class ApiSetPriceAlertDetailController extends \crocodicstudio\crudbooster\controllers\ApiController {

		    function __construct() {    
				$this->table       = "master_users_price_alerts_detail";        
				$this->permalink   = "set_price_alert_detail";    
		    ...
		    public function hook_after($postdata,&$result) {
		         ...
		         if($state_id == 0)
		        {
		        	...
		        	$alert = DB::table("master_users_price_alerts_detail")->select("id")->where("city",$city_id)->where("strain",$postdata["strain"])->where("user_id",$user_id)->first();
		    	}
		    	else
		    	{
		    		$alert = DB::table("master_users_price_alerts_detail")->select("id")->where("state",$state_id)->where("strain",$postdata["strain"])->where("user_id",$user_id)->first();
		    	}	
		       ...
		       // print_r($alert);exit;
		        ...
		        if(isset($alert) && isset($alert->id)){
		        	...
		        	$new_data["status"] = 1;
		        	$new_data["price"] = $postdata["price"];
		        	$new_data["action"] = $postdata["action"];
		        	...
		        	if(count($current_price) > 0){
			        	...
			        	$id = DB::table("master_users_price_alerts_detail")->insertGetId($new_data);
			        	$result["id"] = $id;
		        	}else{
