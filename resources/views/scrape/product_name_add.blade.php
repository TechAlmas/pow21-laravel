	...
	<div>
		...
		<div class='panel panel-default'>
			...
			<div class="panel-body">
				<div class="">
					<div class="col-12">
						<form method="post" action="{{CRUDBooster::mainpath('add-product-name')}}" autocomplete="off" enctype="multipart/form-data">
							@csrf
							<div class="row">
							...
							<div class="row">
								<div class="col-sm-10 form-group{{ $errors->has('product_brand') ? ' has-danger' : '' }}">
									...
									<select name="product_brand" class="form-control select2_option{{ $errors->has('source') ? ' is-invalid' : '' }}" id="product_brand" >
										...
										<option value="{{ $product_brand->brand_id }}" >{!! $product_brand->brand->name !!}</option>
										@endforeach
									</select>
								</div>
							</div>
